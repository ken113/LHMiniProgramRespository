		...
		<view v-for="(element,index) in initFormData.elements" :key="index"
			...
			<view class="uib-item">
				...
				<view class="item-right">
					<input class="input" :disabled="orderDetail.CustomerState !== 0 && orderDetail.CustomerState !== 20" :placeholder="element.tips" v-model="form[index].value"
						@blur="changeInput" />
				</view>
			</view>
		...
		methods: {
			...
			changeInput(e) {
				...
				if (this.initFormData.mandatory === '1') {
					if (value) {
						if (this.initFormData.elements[0].code === 'FlightNo') {
